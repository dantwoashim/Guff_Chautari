...
export interface RuntimeKnowledgeStateSnapshot {
  id: string;
  userId: string;
  workspaceId: string;
...
const toSnapshot = (row: unknown): RuntimeKnowledgeStateSnapshot => {
...
export class RuntimeKnowledgeStateRepository {
  ...
  async loadState(payload: {
    ...
    workspaceId: string;
    namespaceUserId: string;
  }): Promise<RuntimeKnowledgeStateSnapshot | null> {
    const { data, error } = await this.client
      .from('runtime_knowledge_state')
      .select('*')
  ...
  async saveState(payload: {
    ...
    const { error } = await this.client.from('runtime_knowledge_state').upsert({
...
export const runtimeKnowledgeStateRepository = new RuntimeKnowledgeStateRepository();
